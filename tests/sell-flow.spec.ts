...
// Pick an owned card outside the saved deck so the sell button is enabled.
const sellableExtraCardId = cards.find((card) =>
  PROFILE_OWNED_CARD_IDS.includes(card.id) &&
  !PROFILE_DECK_IDS.includes(card.id)
)?.id;
// First deck card is locked (it's in the saved deck).
const inDeckCardId = PROFILE_DECK_IDS[0];

test("Collection card detail sells a duplicate card, decrements badge, and credits the HUD crystals", async ({ page }) => {
  if (!sellableExtraCardId || !PROFILE_OWNED_CARD_IDS.includes(sellableExtraCardId)) {
...
test("Collection sell-all duplicates keeps one copy in the collection", async ({ page }) => {
  if (!sellableExtraCardId) throw new Error("Sell-flow fixture requires an owned card outside the saved deck.");
  const sellableCard = cards.find((card) => card.id === sellableExtraCardId);
  if (!sellableCard) throw new Error(`Card ${sellableExtraCardId} must exist in card pool.`);

  const sellIdentity: PlayerIdentity = { mode: "guest", guestId: "guest-sell-all-duplicates-e2e" };
  let lastSellCount = 0;
  let currentExtraCount = 3;
  ...
  await page.route("**/api/player/sell", async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        player: {
          id: "player-sell-all-duplicates-e2e",
          identity: body.identity,
          ownedCards,
          deckIds: PROFILE_DECK_IDS,
          starterFreeBoostersRemaining: 0,
          openedBoosterIds: ["neon-breach", "factory-shift"],
          crystals: body.count * SELL_PRICES_BY_RARITY[sellableCard.rarity],
          totalXp: 0,
          level: 1,
          wins: 0,
          losses: 0,
